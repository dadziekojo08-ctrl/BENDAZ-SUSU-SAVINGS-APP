...
export const BankerPortal: React.FC<BankerPortalProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-2xs space-y-3">
        ...
        <div className="space-y-2">
          {bankerTodayTransactions.length === 0 ? (
            ...
            bankerTodayTransactions.slice(0, 8).map((tx) => {
              const isVoided = tx.status === 'VOIDED';
              return (
                <div
                  key={tx.id}
                  className={`p-2.5 rounded-xl transition-colors flex items-center justify-between text-xs ${
                    isVoided
                      ? 'bg-rose-50/50 border border-rose-100 opacity-75'
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span className={`font-bold ${isVoided ? 'line-through text-slate-400' : 'text-slate-800'}`}>
                          {tx.memberName}
                        </span>
                        {isVoided && (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!isVoided && (
                      <button
                        ...
                        className="px-2 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-[10px] font-bold flex items-center gap-1 cursor-pointer transition-colors shadow-2xs"
                        title="Undo accidental entry (Admin Override required)"
                      >
      ...
      {/* Void Transaction Modal (Admin Override) */}
